...
* @n: A pointer to the queue's size value.* @item: The item to add to the queue.
*/
	...
	if (tree != NULL)
	{
		...
		while (n > 0)
		{
			...
			else
			{
				...
				{
					enqueue_item_2(&queue_head, &queue_tail, &n, (void *)(current->parent->left));
					enqueue_item_2(&queue_head, &queue_tail, &n, (void *)(current->parent->right));
				}
			}
