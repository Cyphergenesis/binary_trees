...
heap_t *get_last_heap_node(const heap_t *root)
{
	...
	if (root != NULL)
	{
		enqueue_item_4(&head, &tail, &k, (void *)root);
		while (k > 0)
		{
			current = head;
...
/**
* swap_tree_node_with_parent_1 - Swaps a node in a \* max binary heap tree with its parent.
	...
	if ((node != NULL) && ((*node)->parent != NULL) && (root != NULL))
	{
		...
		if ((*node)->parent->left != *node) /* swap from the right */
		{
			...
			if ((*node)->parent->left != NULL)
				...
				(*node)->parent = tmp_0, (*node)->left = m->left, (*node)->right = m;
				m->parent = node_cpy, m->left = l, p->right = r;
		}
		else /* swap from the left */
		{
			if ((tmp_0 != NULL) && (tmp_0->left == m))
				tmp_0->left = *node;
			if ((tmp_0 != NULL) && (tmp_0->right == m))
				tmp_0->right = *node;
			if ((*node)->parent->right != NULL)
				(*node)->parent->right->parent = *node;
			(*node)->parent = tmp_0, (*node)->right = m->right, (*node)->left = m;
			m->parent = node_cpy, m->left = l, m->right = r;
		}
		if (l != NULL)
			l->parent = p;
		if (r != NULL)
